...
function saveDailyMood() {
  ...
  // 🆕 Отправляем данные на сервер — ✅ КЛЮЧ `data` ВОССТАНОВЛЕН
...
// Diary View — ✅ ИСПРАВЛЕНО: ключи `data` на месте
...
// Diary View — ✅ ИСПРАВЛЕНО: добавлены ключи `data`
function updateDiaryView() {
  const diaryEntries = document.getElementById("diary-entries")
  diaryEntries.innerHTML = ""
  ...
  // Add mood entries — ✅ КЛЮЧ `data` ДОБАВЛЕН
  Object.values(appState.moodEntries).forEach((entry) => {
    allEntries.push({
      date: new Date(entry.date),
      type: "mood",
  ...
  // Add day notes — ✅ КЛЮЧ `data` ДОБАВЛЕН
  Object.entries(appState.dayNotes).forEach(([dateStr, note]) => {
    allEntries.push({
      date: new Date(dateStr),
      type: "note",
...
function saveDailyMood() {
  ...
  // 🆕 Отправляем данные на сервер для анонимного хранения
  const userId = window.Telegram?.WebApp?.initDataUnsafe?.user?.id?.toString() || "anonymous";
  const userData = {
    type: "mood",
    payload: appState.moodEntries[today],
    userId: userId,
    timestamp: new Date().toISOString(),
  };
